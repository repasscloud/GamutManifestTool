...
function New-GamutManifest {
    ...
    begin {
        ...
        [String]$dir_Manifests=Join-Path -Path $OutPath -ChildPath 'app'
        if (-not (Test-Path -Path $dir_Manifests) ) {
            New-Item -Path $OutPath -ItemType Directory -Name 'app' -Confirm:$false -Force
        }
        ...
        $zString=$zClient.DownloadString('https://raw.githubusercontent.com/repasscloud/software-library/master/lib/public/json_copyright_notice.json')
